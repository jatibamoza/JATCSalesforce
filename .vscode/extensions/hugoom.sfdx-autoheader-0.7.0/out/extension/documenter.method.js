...
const templates_method_1 = require("../templates/templates.method");
...
class MethodDocumenter {
     ...
     */
    insertMethodHeaderFromCommand(editor, edit) {
        const methodSelection = editor.selection;
        const currentLineId = methodSelection.anchor.line;
        const lineToInsertId = this.getMethodFirstLineIncludingAnnotations(editor.document, currentLineId);
        if (lineToInsertId === null) {
            vscode_1.window.showErrorMessage("SFDoc: Method comment already present.");
            return;
        }
        if (methodSelection.active.line !== currentLineId) {
            vscode_1.window.showErrorMessage("SFDoc: Multiline selection is not supported. Set the cursor's position on the first line of the method's declaration.");
            return;
        }
        const methodHeader = this.constructMethodHeader(currentLineId);
        if (!methodHeader) {
            vscode_1.window.showErrorMessage("SFDoc: Apex method's signature not recognized on selected line.");
            return;
        }
        edit.insert(new vscode_1.Position(lineToInsertId, 0), methodHeader);
    }
    /**
     ...
     */
    getMethodFirstLineIncludingAnnotations(document, methodFirstLineId) {
        let methodFirstLineBeforeAnnotations = methodFirstLineId;
        while (documenter_helper_1.default.apexAnnotationsRegex.test(document.lineAt(methodFirstLineBeforeAnnotations - 1).text))
            methodFirstLineBeforeAnnotations--;
        // TODO: Move to its own method "checkForMethodHeader"
        return /\/\/|\*\//i.test(document.lineAt(methodFirstLineBeforeAnnotations - 1).text)
            ? null
            : methodFirstLineBeforeAnnotations;
    }
    /**
     ...
     *  will be generated
     */
    constructMethodHeader(currentLineId) {
        if (!vscode_1.window.activeTextEditor)
            return;
        const method = this.parseApexMethodSignature();
        if (!method)
            return;
        const str = templates_method_1.getMethodHeaderFromTemplate(method.parameters, method.returnType);
        const indentation = vscode_1.window.activeTextEditor.document
     ...
     */
    matchIndentation(methodHeaderString, indentation) {
        if (!indentation)
            return methodHeaderString;
        return methodHeaderString
            .split(/\n/gim)
            .reduce((lines, line) => line ? (lines += indentation + line + "\n") : lines, "");
    }
    /**
     * Parse the signature of the Apex method being process into the TS "Method" type.
     */
    parseApexMethodSignature() {
        if (!vscode_1.window.activeTextEditor)
            return;
        const method = {
            parameters: [],
        ...
        let currentLineId = vscode_1.window.activeTextEditor.selection.anchor.line;
        if (!document.lineAt(currentLineId).text.replace(/\s/gi, "")) {
            vscode_1.window.showErrorMessage("SFDoc: Cannot insert method header on empty line.");
            return;
        }
        ...
        method.returnType =
            signatureTokens.find((token) => !documenter_helper_1.default.apexReservedTerms.includes(token.toLowerCase()) &&
                !documenter_helper_1.default.apexAnnotationsRegex.test(token)) || "";
        if (!method.returnType) {
            const isMethodAConstructror = documenter_helper_1.default.getContainingClassName(document, currentLineId) === method.name;
            if (isMethodAConstructror)
        ...
        const isIncludeParameterType = vscode_1.workspace
        ...
        if (!isIncludeParameterType)
            method.parameters = method.parameters.map((token) => token.split(" ").pop() || "");
        return method;
    }
    /**
     ...
     */
    tokenizeApexMethod(content, isSignature) {
        ...
        while (!iterVal.done) {
            processedTokens.push(this.processToken(iterVal.value, tokensIterator, isSignature, false, isSignature));
     ...
     */
    processToken(token, tokensIterator, isFirstOfLine, isWithinCollection, isSignature = false) {
        let currentProcessedToken = "";
        if (!token)
            return currentProcessedToken;
        if (token.toLowerCase() === "map")
            currentProcessedToken = this.processTokenMap(token, tokensIterator, isWithinCollection, isSignature);
        else if (token.toLowerCase() === "list" || token.toLowerCase() === "set")
            currentProcessedToken = this.processTokenListOrSet(token, tokensIterator, isWithinCollection, isSignature);
        else if (!isFirstOfLine)
            currentProcessedToken =
                token +
                    this.processToken(tokensIterator.next().value, tokensIterator, true, false);
        else
            currentProcessedToken = (isWithinCollection ? "" : " ") + token;
        return currentProcessedToken;
    }
    /**
     ...
     *  A false value indicates that method parameters token are being processed instead
     */
    processTokenMap(token, tokensIterator, isWithinCollection, isSignature) {
        let processedToken = token +
            "<" +
            this.processToken(tokensIterator.next().value, tokensIterator, true, true) +
            ", " +
            this.processToken(tokensIterator.next().value, tokensIterator, true, true) +
            ">";
        if (!isWithinCollection && !isSignature)
            processedToken += this.processToken(tokensIterator.next().value, tokensIterator, true, false);
     ...
     *  A false value indicates that method parameters token are being processed instead
     */
    processTokenListOrSet(token, tokensIterator, isWithinCollection, isSignature) {
        let processedToken = token +
            "<" +
            this.processToken(tokensIterator.next().value, tokensIterator, true, true) +
            ">";
     ...
     */
    getMethodTextFromDocument(lineNumber, document) {
        const re_methodDefinitionEnd = /\)/gi;
        let methodText = "";
        while (true) {
            let currentLine = document.lineAt(lineNumber).text;
            methodText += currentLine;
            if (re_methodDefinitionEnd.test(methodText))
                break;
            if (/\*\/|\/\*|\bclass\b|\}/gi.test(currentLine))
                return;
...
exports.default = MethodDocumenter;
//# sourceMappingURL=documenter.method.js.map