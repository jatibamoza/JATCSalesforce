...
const { workspace, Position, WorkspaceEdit, TextEdit, Range, extensions, window, Selection, } = require("vscode");
...
suite("Salesforce Documenter - Extension Suite", function () {
    ...
    test("Testing PostSaveListener - Cursor Reset", async () => {
        ...
        edit.insert(document.uri, getEOFPosition(document), `public class testContent {
  //Cursor Position Test
    ...
    test("Testing getInsertFileHeaderEdit - Javalike", async () => {
        const document = await openTestDocumentByFileIdentifier("apex");
        const insertFileHeaderEdit = await fileDocumenter.getInsertFileHeaderEdit(document);
    ...
    */`;
    ...
    test("Testing updateLastModifiedBy", (done) => {
        ...
        assert.notStrictEqual(fileDocumenter.updateLastModifiedBy(testModByString), testModByString);
        done();
    });
    test("Testing updateLastModifiedDateTime", (done) => {
        const testModOnString = "* @Last Modified On: 02/02/2222 22:22";
        assert.notStrictEqual(fileDocumenter.updateLastModifiedDate(testModOnString), testModOnString);
        done();
    });
...
async function openTestDocumentByFileIdentifier(ext) {
    ...
    const doc = await workspace.openTextDocument(path.join(__dirname, "../../test_files/", "testFile_SFDXAutoheader", fileIdentifierAssociation[ext]));
...
function loadExtension() {
    ...
    if (testExt.isActive)
        return Promise.resolve(testExt.exports);
